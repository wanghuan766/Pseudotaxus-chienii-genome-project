#!/bin/bash
#SBATCH --job-name=gff_pipeline_hap2
#SBATCH --partition=SMP
#SBATCH --nodes=1
#SBATCH --cpus-per-task=48
#SBATCH --output=gene_anno_hap2_%j.log
...
# ==============================================================================
THREADS=48
GENOME="../01_Genome_Assembly/Pseudotaxus_chienii.hap2.fasta"
MASKED_GENOME="../03_Repeat_Annotation/Pseudotaxus_chienii.hap2.fasta.masked"
...
echo "=== [START] ALL-IN-ONE STRUCTURAL ANNOTATION PIPELINE FOR HAP2 ==="
...
fastp -i ${FASTQ_DIR}/RNA_R1.fastq.gz -I ${FASTQ_DIR}/RNA_R2.fastq.gz \
      -o RNA_clean_R2_1.fastq.gz -O RNA_clean_R2_2.fastq.gz \
      --thread 16 --html fastp_hap2.html

echo "--- [STAGE 1] Building HISAT2 Index ---"
hisat2-build -p ${THREADS} ${GENOME} P_chienii_hap2_idx

echo "--- [STAGE 1] Aligning RNA-seq reads via HISAT2 ---"
hisat2 -p ${THREADS} -x P_chienii_hap2_idx \
       -1 RNA_clean_R2_1.fastq.gz -2 RNA_clean_R2_2.fastq.gz \
       -S P_chienii_hap2_rnaseq.sam

echo "--- [STAGE 1] Sorting and indexing BAM file ---"
samtools sort -@ ${THREADS} -o aligned_merged_hap2.bam P_chienii_hap2_rnaseq.sam
samtools index aligned_merged_hap2.bam
rm P_chienii_hap2_rnaseq.sam # Clean up large intermediate file

echo "--- [STAGE 1] Transcriptome assembly via StringTie ---"
stringtie aligned_merged_hap2.bam \
          -p ${THREADS} \
          -o P_chienii_hap2_stringtie.gtf \
          -v
...
braker.pl --genome=${MASKED_GENOME} \
          --bam=aligned_merged_hap2.bam \
          --prot_seq=${PROTEINS} \
          --threads=${THREADS} \
          --workingdir=braker3_hap2_out \
          --gff3
...
$EVM_DIR/EvmUtils/write_EVM_inputs.pl \
    ...
    --gene_predictions ./braker3_hap2_out/braker.gff3 \
    --protein_alignments ./gemoma_hap2_Ath_out/gemoma_predictions.gff3 \
    --transcript_alignments ./P_chienii_hap2_stringtie.gtf \
    --output_file_pre evm_input_hap2

echo "--- [STAGE 4] Running final EvidenceModeler consensus integration ---"
$EVM_DIR/EvidenceModeler --model_org_type OTHER \
                         --genome ${MASKED_GENOME} \
                         --weights ./weights_hap2.txt \
                         --gene_predictions ./braker3_hap2_out/braker.gff3 \
                         --protein_alignments ./gemoma_hap2_Ath_out/gemoma_predictions.gff3 \
                         --transcript_alignments ./P_chienii_hap2_stringtie.gtf \
                         --segmentSize 500000 --overlapSize 10000 --write_executions \
                         --output_file Pseudotaxus_chienii_hap2_EVM_final.gff3

echo "=== [FINISHED] COMPLETE STRUCTURAL ANNOTATION PIPELINE FOR HAP2 ==="