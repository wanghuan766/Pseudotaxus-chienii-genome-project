#!/bin/bash
#SBATCH --job-name=func_pipeline_hap2
#SBATCH --partition=SMP
#SBATCH --nodes=1
#SBATCH --cpus-per-task=48
#SBATCH --output=func_anno_hap2_%j.log
...
# ==============================================================================
THREADS=48
PROTEINS="../04_Gene_Annotation/Pseudotaxus_chienii_hap2_EVM_final.proteins.fa"
...
echo "=== [START] ALL-IN-ONE FUNCTIONAL ANNOTATION PIPELINE FOR HAP2 ==="
...
diamond blastp --db ${NR_DB} \
               --query ${PROTEINS} \
               --out P_chienii_hap2_diamond_nr.txt \
               --evalue 1e-5 \
               --top 5 \
               --threads ${THREADS} \
...
diamond blastp --db ${SWISSPROT_DB} \
               --query ${PROTEINS} \
               --out P_chienii_hap2_diamond_swissprot.txt \
               --evalue 1e-5 \
               --top 5 \
               --threads ${THREADS} \
               --outfmt 6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore stitle
...
interproscan.sh -i ${PROTEINS} \
                -f TSV \
                -b P_chienii_hap2_interproscan_out \
                --applications Pfam \
                -goterms \
                -pa \
                -cpu ${THREADS}

echo "=== [FINISHED] COMPLETE FUNCTIONAL ANNOTATION PIPELINE FOR HAP2 ==="